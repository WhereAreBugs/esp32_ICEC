...
void log(module module, logLevel level, const char *context) {
    ...
    Serial.printf("[%s][%s] %s\n", module == MODULE_MAIN ? "MAIN" : module == MODULE_SERIAL_IO ? "SERIAL_IO" : module == MODULE_STATUS ? "STATUS" : module == MODULE_DISPLAYCORE ? "DISPLAYCORE" : module == MODULE_TEMPERATURE ? "TEMPERATURE" : module == MODULE_PWM ? "PWM" : module == MODULE_ALARM ? "ALARM" : module == MODULE_TIMESET ? "TIMESET" : module == MODULE_LOGSYSTEM ? "LOGSYSTEM" : module == MODULE_DS1307 ? "DS1307" : "UNKNOWN", level == LOG_LEVEL_DEBUG ? "DEBUG" : level == LOG_LEVEL_INFO ? "INFO" : level == LOG_LEVEL_WARNING ? "WARNING" : level == LOG_LEVEL_ERROR ? "ERROR" : "FATAL",context);
